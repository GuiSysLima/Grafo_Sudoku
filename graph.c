...
// Alocação dinâmica de memória para o grafo
...
//Aloca espaço para o vértice no grafo, atribui os valores e insere no grafo
struct vertex *add_vertex(struct graph *graph, int num_possible, int value){

	struct vertex *next = malloc(sizeof(struct vertex));
	//No caso um novo vértice está sendo inserido 
	//Aumenta o indice de vértices.
	next->index = graph->vertex_index++;

	//Recebe da input da função graph *load_initial
	//Atribui os valores passados no parâmetro
